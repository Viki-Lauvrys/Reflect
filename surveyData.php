...
function changeBackground() {
    if (localStorage.getItem('colour')) {
        ...
        // Change slider color
        var styleSheet = document.styleSheets[0]; // Assuming the stylesheet is the first one
        var rules = document.getElementsByClassName("range");
        for (var j = 0; j < rules.length; j++) {
            var rule = rules[j];
            var cssRule = '.range::-webkit-slider-thumb { box-shadow: -500px 0 0 500px' + adjust(colorrr, -15) + '; }';
            styleSheet.insertRule(cssRule, styleSheet.cssRules.length);
        }

        // Change scrollbar color
        var bars = document.getElementsByClassName("scrollbar1");
        for (var j = 0; j < bars.length; j++) {
            var bar = bars[j];
            var cssRule = '.scrollbar1::-webkit-scrollbar-thumb { background-color: ' + adjust(colorrr, -50) + '; }';
            styleSheet.insertRule(cssRule, styleSheet.cssRules.length);
        }
            ...
            <div id="sides-container" class="fadeIn" style="display:none;">
                <div id="left-side">
                    ...
                    <form action="pdfMaker/pdfMaker.php?" method="get" target="_blank">
                        ...
                        <input id="inputGroup" class="input2" placeholder="Klas" type="text" name="inputGroup" />
                ...
                <div id="right-side" class="scrollbar1">
                    <div id="titleWrapper">
                        <h1>Mijn modules</h3>
                    </div>
                    <div id="print-this">
                    ...
                    echo "<div class='title'>" . utf8_decode(htmlspecialchars($row2['title'])) . "</div>";
                    echo "<div class='info'>" . utf8_decode(htmlspecialchars($row3['info'])) . "</div> <br/>";
                                                                $siid = $row3['siid'];
