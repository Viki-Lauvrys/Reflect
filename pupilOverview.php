  ...
  <body>
    ...
    function changeChart(year) {
        ...
        if (year === 'alles') {
            ...
            $.ajax({
                type: "POST",
                url: "ajaxFolder/getOverviewData.php",
                data: { userID: '<?php echo $userID ?>', year: year },
                success: function(response) {
                    var response = JSON.parse(response);
                    options.series[0].data = JSON.parse(response.q1);
                    options.series[1].data = JSON.parse(response.q4);
                    options.series[2].data = JSON.parse(response.q5);
                    var chart = new ApexCharts(document.querySelector("#chart"), options);
                    $('#chart').empty();
                    chart.render();
                }
