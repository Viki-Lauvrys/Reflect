<?php 
            ...
            if($result)
            {
                if($result && mysqli_num_rows($result) > 0)
                {
                    ...
                    if(password_verify($password, $user_data['password']))
                    {
                        ...
                        $result = $conn->query("SELECT groupName FROM smartschool_groups_users WHERE userID = '" . $_SESSION['username'] . "'");
                        if ($result->num_rows > 0) {
                            while($row = $result->fetch_assoc()) {
                                $groupName = $row['groupName'];
                                $_SESSION['groupName_' . $groupName] = $groupName;
                            }
                        }
    ...
    } else if (date('m') >= 1 && date('m') <= 8) {
        $_SESSION['schoolYear'] = date('Y') - 1;
    }
?>
